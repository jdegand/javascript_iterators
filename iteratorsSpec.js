...
describe("#summerJamCount", function(){
  it("should get number of summer songs", function(){
    expect(summerJamCount(songs)).to.equal(47)
  });
});

describe("#getTotalDurationInSeconds", function(){
  it("total duration of all songs in seconds", function(){
    expect(getTotalDurationInSeconds(songs)).to.equal(46236)
  });
});

describe("#getSongCountByArtist", function(){
  it("how many keys in object", function(){
    expect(Object.keys(getSongCountByArtist(songs)).length).to.equal(149)
  });
});
